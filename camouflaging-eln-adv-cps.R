--------------------------------------------------------------------------------
# Elastic-Net Regression Analysis to Identify the Socio-Motivational and 
# Cognitive Predictors of the CAT-Q
--------------------------------------------------------------------------------
...
dfBas <- data.frame(df$CATQ, df$IOS, df$AEFI, df$CFS, df$IRI, df$BIS, df$ERQ, 
                    df$SelfCS, df$INQ, df$INCOM, df$ISMI, df$RSE, df$PSCS, 
                    df$LSAS_anx, df$LSAS_avd, df$ASRS, df$SATQ)

# Method 1: Following Chris' Script---------------------------------------------
# Set up model matrix
response_mat <-
  model.matrix(~ df.IOS + df.AEFI + df.CFS + df.IRI + df.BIS + df.ERQ + 
                 df.SelfCS + df.INQ + df.INCOM + df.ISMI + 
                 df.RSE + df.PSCS + df.LSAS_anx + df.LSAS_avd + df.ASRS +
                 df.SATQ, data = dfBas)[,-1]

response_var <- "df.CATQ"
...
# Full elastic net analyses with 10-fold cross validation
fit_elas <- function(df, response_var, pred_mat){
  cv.glmnet(y = df[[response_var]], x = pred_mat,
            standardize = FALSE, thresh = 1e-10)
}

set.seed(1234)
elas_res <- fit_elas(dfBas, response_var, scaled_response_mat)

# Extract parameters and visualize most important predictors 
elas_res$lambda.1se

coefs <-
  coef(elas_res, s = "lambda.1se", 
       exact = TRUE,
       x = scaled_response_mat,
       y = dfBas[[response_var]])[-1]

vip(elas_res, num_features = 16, geom = "point")

# Selective inference (to get p-value)
inf <-
  fixedLassoInf(
    x = scaled_response_mat,
    y = dfBas[[response_var]],
    coefs,
    elas_res$lambda.1se * nrow(scaled_response_mat)
  )
...
summarize_inference <-
  function(inf){
    cbind(inf$var
          ...
          , inf$coef0/inf$sd
          , inf$pv) %>%
      `colnames<-`(c("var", "coef", "z", "p")) %>%
      as.data.frame %>%
      mutate(q = p.adjust(p, method = "BH")) %>%
      mutate(post_selection_z = -1 * sign(z) * qnorm(p))
  }

# Result
summarize_inference(inf)

# Plot effect size
generate_figure <- function(inference, title, xlab, file = NULL){
  p <-
    summarize_inference(inference) %>% 
    arrange(post_selection_z) %>% 
    mutate(var = factor(as.character(var), levels = var)) %>% 
    ggplot(aes(x = var, y = post_selection_z, colour = q < .05)) + 
    ...
    xlab(xlab) +
    ylab ("Effect Size (z-score)") +
    theme(legend.position="bottom") +
    scale_colour_discrete(guide = "none") +
    annotate("text"
             , x = last_plot()$data %>% with(var[post_selection_z == -Inf])
             , y = last_plot()$data$post_selection_z %>% { min(.[is.finite(.)])}
             , label = "*", size = 6
    ) +
    theme(plot.margin = margin(l = 50, t = 5, r = 5, b = 5)) 
  
  if(!is.null(file))
    ggsave(file, plot = p, width = 8, height = 8)
  
  p
}

p1 <- generate_figure(inf, 
                      "Cognitive Predictors of Camouflaging", 
                      "Cognitive Predictors",
                      "camouflaging-predictors.pdf"
)
...
fit_elas2 <- function(df, response_var, pred_mat){
  train(y = df[[response_var]], x = pred_mat, method = "glmnet", 
...
set.seed(1234)
eln <- fit_elas2(dfBas, response_var, scaled_response_mat)

# Extract parameters and visualize most important predictors 
n = 972 # total sample size 

eln$bestTune 
best_model <- eln$finalModel
beta <- coef(best_model, s = eln$bestTune$lambda) # gives variable names
beta
coefs2 <- coef(best_model, s = eln$bestTune$lambda/n)[-1] # remove intercept
coefs2
...
vip(eln, num_features = 16, geom = "point") 
...
# Result
summarize_inference(inf2)

# Cross-validation--------------------------------------------------------------
loo_model <- function(df, response_var, pred_mat){
  y <- df[[response_var]]
  x <- pred_mat
  sapply(seq_along(y), function(i){
    ...
    res <- fit_elas(train_d, response_var, train_x)
    (test_y - as.vector(predict(res, test_x, s = "lambda.1se")))^2
  })
}

loo <- loo_model(dfBas, response_var, scaled_response_mat)

loo_results <-
  tribble(
    ~ outcome       , ~ predictors, ~ mean      , ~ sd
    , response_var  , "all"       , mean(loo) , sd(loo)
  )
loo_results
